...
func createPlayer(e *engine.Entity, p *std.Vector3, c *engine.Entity, ls []*Level) {
	...
	idleSprite := render.NewTexture(idleAtlas)
	idleSprite.SetSize(11, 17)
	idleSpriteCoord := std.Vector2{0, 0}
	idleSprite.SetOffset(&idleSpriteCoord)

	material.SetTexture(idleSprite)
	...
	boundsCheck := scripting.NewScript()
	boundsCheck.OnUpdate(func() {
		if !ps.Completed() {
			if p.Y >= 90 {
				p.Y = 89
			}
